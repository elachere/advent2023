...
class Map(BaseModel):
    ...
    def parse_number(self, x: int, y: int) -> (int, int):
        ...
        number and the y position of its last digit.
        """
...
def main():
    ...
    while x <= _map.height:
        ...
        while y <= _map.width:
            ...
            if _map(x, y).is_number:
                adj = _map.find_ajdacents(_map(x, y))
                if any(point.value not in not_symbols for point in adj):
                    number, next_y = _map.parse_number(x, y)
